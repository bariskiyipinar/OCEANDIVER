using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.UI;

public class Boss1 : MonoBehaviour
{
    public float BossMoveSpeed = 2f;
    public float MinY = -1.8f;
    public float MaxY = 3.5f;

    public float health = 1000f; // CAN 1000 OLDU
    private float maxHealth = 1000f;
    public Image healthBar; // UI'deki can barý (Image olarak ayarlandý)

    private float direction = 1f;

    public GameObject FishPrefab;
    public float FishPower = 5f;
    public float FishSpawnInterval = 2f;


   
    public Animation BossDeath;

    void Start()
    {
       
        InvokeRepeating("FishEnemy", 2f, FishSpawnInterval);

        if (healthBar != null)
        {
            healthBar.fillAmount = 1f; // Baþlangýçta tamamen dolu
        }
    }

    void Update()
    {
        BossMovement();
    }

    public void FishEnemy()
    {
        if (FishPrefab != null)
        {
            GameObject newFish = Instantiate(FishPrefab, transform.position, Quaternion.identity);
            Rigidbody2D rb = newFish.GetComponent<Rigidbody2D>();

            if (rb != null)
            {
                rb.velocity = Vector2.left * FishPower;
                newFish.transform.localScale = new Vector3(4, 4, 4);
                newFish.transform.localRotation = Quaternion.Euler(0, 180, 0);
            }
        }
    }

    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.CompareTag("Bullet"))
        {
            health -= 100f; // Hasar alýyor
            Debug.Log("Boss Caný: " + health);

            if (healthBar != null)
            {
              
                float HealtBarScale = Mathf.Clamp01(health / maxHealth);
                healthBar.transform.localScale = new Vector3(HealtBarScale, healthBar.transform.localScale.y, healthBar.transform.localScale.z);
            }

            Destroy(collision.gameObject);

            if (health <= 0)
            {
                Die();
            }
        }
    }

    void BossMovement()
    {
        float newY = transform.position.y + (BossMoveSpeed * direction * Time.deltaTime);
        newY = Mathf.Clamp(newY, MinY, MaxY);
        transform.position = new Vector2(transform.position.x, newY);

        if (newY >= MaxY || newY <= MinY)
        {
            direction *= -1;
        }
    }

    void Die()
    {
        Debug.Log("Boss Öldü!");
     
        if (BossDeath != null)
        {
            BossDeath.Play("EnemyDeath"); // "EnemyDeath" animasyonunu oynat
            StartCoroutine(DestroyAfterAnimation());
        }
        else
        {
            Destroy(gameObject); // Eðer animasyon yoksa hemen yok et
        }


    }


    IEnumerator DestroyAfterAnimation()
    {
        yield return new WaitForSeconds(BossDeath.clip.length); // Animasyon süresi kadar bekle
        Destroy(gameObject);
    }



}
